...
import os
import numpy as np
from model.mpf_mdr_filter_fusion_add import MPF_MDR_Model
...
def train(args, model, load_data,optimizer,device,cluster_assignments,cluster_center_dict):
    # logger.info('start train')
    model.train()

    total_loss,total_main_loss, total_cl_loss= [],[],[]
    ...
    for i, data in enumerate(load_data.train_loader, 0):
        ...
        optimizer.zero_grad()
        prob,p_u,h_u,p_d = model.forward(batch_nodes_u.to(device),batch_nodes_v.to(device),batch_nodes_d.to(device),cluster_assignments,cluster_center_dict)
    ...
    # total_cl_loss.append(sum(total_cl_loss)/len(total_cl_loss))
    return total_loss,total_main_loss

def test(args,load_data,model,device,cluster_assignments,cluster_center_dict):
    model.eval()
    with torch.no_grad():
        ...
        (hr_zero_shot, ndcg_zero_shot,mrr_zero_shot,hr_5_zero_shot,ndcg_5_zero_shot,mrr_5_zero_shot,hr_1_zero_shot,ndcg_1_zero_shot,mrr_1_zero_shot) = calculate_hr_ndcg(model=model,test_ratings=pos_samples_zero_shot,test_negatives=neg_samples_zero_shot,
                                           k=args.top_k,device=device,cluster_assignments=cluster_assignments,cluster_center_dict=cluster_center_dict,user_cat='0')
        (hr_few_shot, ndcg_few_shot, mrr_few_shot, hr_5_few_shot, ndcg_5_few_shot, mrr_5_few_shot, hr_1_few_shot,
         ndcg_1_few_shot, mrr_1_few_shot) = calculate_hr_ndcg(model=model, test_ratings=pos_samples_few_shot,
                                                              ...
                                                              cluster_center_dict=cluster_center_dict, user_cat='1')
        (hr_warm_shot, ndcg_warm_shot, mrr_warm_shot, hr_5_warm_shot, ndcg_5_warm_shot, mrr_5_warm_shot, hr_1_warm_shot,
         ndcg_1_warm_shot, mrr_1_warm_shot) = calculate_hr_ndcg(model=model, test_ratings=pos_samples_warm_shot,
                                                                ...
                                                                k=args.top_k, device=device,
                                                                cluster_assignments=cluster_assignments,
                                                                cluster_center_dict=cluster_center_dict, user_cat='2')
...
if __name__ == '__main__':
    ...
    logger.info(f'parameter settings: batch_size:{args.batch_size},lr:{args.lr},prompt_dim:{args.prompt_dim}, multi_modal_dim: {args.multi_modal_dim},'
                f'alpha:{args.alpha},tau:{args.tau},k:{args.k},domain_num:{args.domain_num},domain_names:{args.domain_names}'
    ...
    load_data_params = {
        ...
        'visual_feat_path':f'{path}/{domains}_pretrain/all_domain_image_feature_filter1.npy',
    ...
    # with open('best_model_filter/pretrain/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_add_fusion_cluster_center_dict_10_04.pkl','rb') as f:
    #     cluster_center_dict = pickle.load(f)
    # cluster_assignments = np.load('best_model_filter/pretrain/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_add_fusion_cluster_assignments_10_04.npy')
    # pretrain_dict = torch.load('best_model_filter/pretrain/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_add_fusion_10_04.pth',map_location=lambda storage, loc: storage)  # update net parameters
    with open(
            'best_model/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_cluster_center_dict_10_22.pkl', 'rb') as f:
        cluster_center_dict = pickle.load(f)
    cluster_assignments = np.load(
        'best_model/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_cluster_assignments_10_22.npy')
    pretrain_dict = torch.load(
        'best_model/health_cloth_beauty_b_2048_d_256_alpha_0.2_k_5_aug_noise_10_22.pth', map_location=lambda storage, loc: storage)
    ...
    for epoch in range(1, args.epochs + 1):
        if endure_num >= 5:
            logger.info('End')
            break
        # total_loss,total_main_loss= train(args, model, load_data,optimizer,device,cluster_assignments,cluster_center_dict)
        # epoch_loss = sum(total_loss)/len(total_loss)
        # epoch_main_loss = sum(total_main_loss)/len(total_main_loss)
        # # epoch_cl_loss = sum(total_cl_loss)/len(total_cl_loss)
        # logger.info('epoch {}, training total loss is {}, main loss is {}'.format(epoch,epoch_loss,epoch_main_loss))
        (hr_zero_shot, ndcg_zero_shot, mrr_zero_shot, hr_5_zero_shot, ndcg_5_zero_shot, mrr_5_zero_shot, hr_1_zero_shot,ndcg_1_zero_shot, mrr_1_zero_shot,
         hr_few_shot, ndcg_few_shot, mrr_few_shot, hr_5_few_shot, ndcg_5_few_shot, mrr_5_few_shot, hr_1_few_shot,ndcg_1_few_shot, mrr_1_few_shot,
         hr_warm_shot, ndcg_warm_shot, mrr_warm_shot, hr_5_warm_shot, ndcg_5_warm_shot, mrr_5_warm_shot, hr_1_warm_shot,ndcg_1_warm_shot, mrr_1_warm_shot) = test(args,load_data,model,device,cluster_assignments,cluster_center_dict)
