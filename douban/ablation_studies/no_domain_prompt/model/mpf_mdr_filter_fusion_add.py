...
class MPF_MDR_Model(nn.Module):
    ...
    def forward(self, nodes_u, nodes_v, nodes_d, cluster_assignments, cluster_center_dict):
        ...
        # cluster_assignments, cluster_center_dict = self.learn_general_multi_interest()
        general_user_interest_list = []
        inter_items_cache = {user: torch.tensor(self.user_inter_dict[user], device=self.device) for user in
                             nodes_u.tolist()}
        scale = torch.sqrt(torch.tensor(self.multi_modal_dim, dtype=torch.float32, device=self.device))
        # logger.info('process inter data')
        batch_users = nodes_u.tolist()
        batch_items = nodes_v.tolist()
        for i in range(len(batch_users)):
            each_user = batch_users[i]
            each_item = batch_items[i]
            # extract inter item multi_modal features
            inter_items = torch.tensor([x for x in inter_items_cache[each_user].tolist() if x != each_item])
            # inter_items_text_emb = self.text_features[inter_items]
            # inter_items_visual_emb = self.visual_features[inter_items]
            # inter_items_multi_modal_emb = torch.cat((inter_items_text_emb, inter_items_visual_emb), dim=0)  # key,value
            # extract cluster centroids including inter items
            inter_items_indices = inter_items.tolist()
            inter_items_clusters = []
            inter_items_text_feat_clusters = [cluster_assignments[item] for item in inter_items_indices]
            inter_items_visual_feat_clusters = [cluster_assignments[item + self.item_num] for item in
                                                inter_items_indices]
            inter_items_clusters.extend(inter_items_text_feat_clusters)
            inter_items_clusters.extend(inter_items_visual_feat_clusters)
            unique_clusters = list(set(inter_items_clusters))
            cluster_centroids = torch.stack(
                [torch.tensor(cluster_center_dict[cid], device=self.device) for cid in unique_clusters])
            user_gen_int = torch.mean(cluster_centroids, dim=0).unsqueeze(0)
            # # Attention mechanism
            # if cluster_centroids.size(0) == 0:
            #     user_gen_int = torch.mean(inter_items_multi_modal_emb, dim=0, keepdim=True)
            # else:
            #     attention_scores = torch.matmul(cluster_centroids, inter_items_multi_modal_emb.T) / scale
            #     attention_weights = F.softmax(attention_scores, dim=-1)
            #     attention_output = torch.matmul(attention_weights, inter_items_multi_modal_emb)
            #     user_gen_int = torch.mean(attention_output, dim=0).unsqueeze(0)
            general_user_interest_list.append(user_gen_int)
        # logger.info('done')
        h_u = torch.cat(general_user_interest_list, dim=0)
        e_t = self.text_features[nodes_v]
        e_v = self.visual_features[nodes_v]
        # h_u_hat = torch.cat([p_u, h_u], dim=1)
        h_u_hat = p_u+h_u
    ...
    def learn_general_multi_interest(self):
        ...
        valid_clusters = {cid: cluster_centers[cid] for cid, domains in domain_per_cluster.items() if len(domains) >= 2}
