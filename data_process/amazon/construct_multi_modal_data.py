...
if __name__ == '__main__':
    ...
    df_inter = pd.read_csv('../datasets/amazon_review/phone_sport_cloth/cloth/cloth_inter_non_overlap_filter.csv')
    item_ids = list(df_inter['item_id'].sort_values())
    process_multi_modal_data(
        meta_data_path='/data/lwang9/datasets/amazon/meta_features/meta_Clothing_Shoes_and_Jewelry.json.gz',
        items=item_ids, to_path='../datasets/amazon_review/phone_sport_cloth/cloth/cloth_multi_modal_data_filter.csv')

    df_inter = pd.read_csv('../datasets/amazon_review/phone_sport_cloth/phone/phone_inter_non_overlap_filter.csv')
    item_ids = list(df_inter['item_id'].sort_values())
    process_multi_modal_data(
        meta_data_path='/data/lwang9/datasets/amazon/meta_features/meta_Cell_Phones_and_Accessories.json.gz',
        items=item_ids, to_path='../datasets/amazon_review/phone_sport_cloth/phone/phone_multi_modal_data_filter.csv')
