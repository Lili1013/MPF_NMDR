...
from model.mpf_mdr_filter_single_v4 import MPF_MDR_Model
...
def train(args, model, load_data,optimizer,device):
    ...
    for i, data in enumerate(load_data.train_loader, 0):
        ...
        prob,p_u,h_u,p_d = model.forward(batch_nodes_u.to(device),batch_nodes_v.to(device),
                                         batch_nodes_d.to(device),top_n_index,
                                         prototypes)
...
if __name__ == '__main__':
    ...
    multi_modal_path = f'/data/lwang9/CDR_data_process/data_process_MPF_MDR/datasets/amazon_review'
    load_data_params = {
        'rating_path': f'{path}/{domains}_single/{dataset}/{dataset}_inter_filter.csv',
        'text_feat_path': f'{path}/{domains}_single/{dataset}/{dataset}_text_features.npy',
        'visual_feat_path': f'{path}/{domains}_single/{dataset}/{dataset}_image_features.npy',
        'train_path': f'{path}/{domains}_single/{dataset}/train_filter.csv',
        'test_path': f'{path}/{domains}_single/{dataset}/test.csv',
        'args': args,
        # 'test_neg_path': f'{path}/{domains}_single/{dataset}/test.txt',
        'test_neg_path_zero_shot': f'{path}/{domains}_single/{dataset}/test_zero_shot_filter.txt',
        'test_neg_path_few_shot': f'{path}/{domains}_single/{dataset}/test_few_shot_filter.txt',
        'test_neg_path_warm_shot': f'{path}/{domains}_single/{dataset}/test_warm_shot_filter.txt',
    }
    ...
    for epoch in range(1, args.epochs + 1):
        if endure_num >= 5:
            logger.info('End')
            break
        total_loss,total_main_loss,top_n_index,prototypes = train(args, model, load_data,optimizer,device)
        ...
        (hr_zero_shot, ndcg_zero_shot, mrr_zero_shot, hr_5_zero_shot, ndcg_5_zero_shot, mrr_5_zero_shot, hr_1_zero_shot, ndcg_1_zero_shot, mrr_1_zero_shot,
         hr_few_shot, ndcg_few_shot, mrr_few_shot, hr_5_few_shot, ndcg_5_few_shot, mrr_5_few_shot, hr_1_few_shot, ndcg_1_few_shot, mrr_1_few_shot,
         hr_warm_shot, ndcg_warm_shot, mrr_warm_shot, hr_5_warm_shot, ndcg_5_warm_shot,
         mrr_5_warm_shot, hr_1_warm_shot, ndcg_1_warm_shot, mrr_1_warm_shot) = test(args,load_data,model,device,top_n_index,prototypes)
        ...
        if hr > best_hr:
            ...
            #            f'delta_{args.delta}_k_{args.k}_n_{args.n}_11_17.pth')
            # np.save(
            ...
            # with open(
            #         f'best_model_v4/{domains}_b_{args.batch_size}_d_{args.prompt_dim}_'
            #            f'delta_{args.delta}_k_{args.k}_n_{args.n}_cluster_center_dict_11_16.pkl',
            #         'wb') as f:
            #     pickle.dump(cluster_center_dict, f)
        else:
            endure_num += 1
