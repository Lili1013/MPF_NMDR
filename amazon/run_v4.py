...
def train(args, model, load_data,optimizer,device):
    ...
    return total_loss,total_main_loss,top_n_index,prototypes,cluster_center_dict
...
if __name__ == '__main__':
    ...
    load_data_params = {
        ...
        'train_path':f'{path}/{domains}_pretrain/train_filter.csv',
        ...
        'args':args,
        'test_neg_path_zero_shot':f'{path}/{domains}_pretrain/test_zero_shot_filter.txt',
        'test_neg_path_few_shot': f'{path}/{domains}_pretrain/test_few_shot_filter.txt',
        'test_neg_path_warm_shot': f'{path}/{domains}_pretrain/test_warm_shot_filter.txt',
    }
    ...
    for epoch in range(1, args.epochs + 1):
        if endure_num >= 5:
            logger.info('End')
            break
        total_loss,total_main_loss,top_n_index,prototypes,cluster_center_dict = train(args, model, load_data,optimizer,device)
        ...
        if hr > best_hr:
            ...
            with open(f'best_model_v4/{domains}_b_{args.batch_size}_d_{args.prompt_dim}_'
                       f'delta_{args.delta}_k_{args.k}_n_{args.n}_cluster_center_dict_01_05.pkl','wb') as f:
                pickle.dump(cluster_center_dict,f)
            torch.save(model.state_dict(),
                       f'best_model_v4/{domains}_b_{args.batch_size}_d_{args.prompt_dim}_'
                       f'delta_{args.delta}_k_{args.k}_n_{args.n}_01_05.pth')
            # np.save(
